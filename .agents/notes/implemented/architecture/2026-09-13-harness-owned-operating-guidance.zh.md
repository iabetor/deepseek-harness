...
文本把四条习惯表述为一条纪律：因环境原因已失败的命令会以同样方式再次失败，因此不要重跑以确认——报告该限制，或先说明什么变了；时长未知或较长的命令应放到后台，让独立工作继续；长命令的输出先落盘再交给任何过滤器读取，这样之后就该输出提出的第二个问题就不必再付一次运行的代价；彼此独立的工具调用放在同一步发出，而不是每步一个，因为串行调用会各自再付一次延迟。每句在加入前都在真实会话中实测过——第三句来自一次因管道输出被丢弃而花掉三轮全量测试与两次快照刷新的会话；第四句来自一次 455 步的会话，其中 190 步只发出一个调用，且有 28 段连续的单 `bash` 步覆盖了 96 步、673 秒本可独立并行的工作，而实测每次往返的固定成本为 5.83 秒。规则保持简短，因为它会在每次请求中渲染，并与 persona 争夺模型的注意力。

递归搜索的范围引导归属于拥有它的工具，而非此处。`dsh-tool-fs-search` 说明递归的 shell 搜索会读取它能触达的每一个字节，包括它自己的 ripgrep 调用会跳过的版本控制与构建目录。该规则是按工具划分的，因此[提示词归属决策](2026-07-05-prompt-variables-and-tool-guidance-ownership.zh.md)将它分配给 `grep` section；一次实测会话在包含 13 GB `.git` 的 25 GB 工作区根目录上执行了三次递归 shell 搜索、耗去 115 秒，而同样搜索经由该工具只需 58 毫秒。
