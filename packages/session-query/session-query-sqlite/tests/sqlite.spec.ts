...
describe('SQLite session search', () => {
  ...
  it('ignores a deletion broadcast while the index is unopened or already closed', async () => {
    // An unopened index has nothing to purge and reconciles on first search, so
    // the purge must not open the database just to drop rows that never existed.
    const cold = await liveContext({ path: ':memory:', openAt: 'first-search' })
    expect(() => { cold.emit('sessionPersistence:deleted', SessionId('never-indexed')) }).not.toThrow()

    // A closed index is equally out of scope: its rows died with the database.
    const closed = await liveContext()
    await (closed.sessionQuery as SqliteSessionQueryEngine).close()
    expect(() => { closed.emit('sessionPersistence:deleted', SessionId('closed')) }).not.toThrow()
  })
