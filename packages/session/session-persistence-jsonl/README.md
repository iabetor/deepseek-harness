...
- **Nothing expires session files** — `destroy` removes one session's directory (its log generation and stray temp links) on request, implementing the seam's `destroy`; there is no automatic retention or pruning, so untargeted logs accumulate under `root`.
