...
本包通过公开的 `sessionStats` 值，为客户端提供全会话轮次与步骤计数，以及 LLM、工具、首 token 和解码墙钟时间，并给出按工具名拆分的工具用时明细。这些数字来自完整的持久日志，因此分页与压缩不会改变它们。当客户端必须在重新加载或缩减历史记录后显示一致的会话统计时，请使用本包。全会话统计不可用时，客户端可改用窗口口径计数。
...
| `tools` | `toolMs` 的按工具名明细：`{ name, calls, ms }` 条目，按 `ms` 降序排列 |
| `ttftMs` / `ttftSteps` | 首 token 延迟之和及其承载步数 |
| `decodeMs` / `decodeTokens` | 上报用量的步的解码墙钟时间与提供方输出 token 之和 |

每个字段在首个贡献事件之前均为 0（`tools` 为空数组）；已装配的注册表恒提供该键，因此客户端读取值本身，而非键的存在性。`tools` 条目只出现在至少有一个已结算配对的工具名上，其 `name` 是 `tool/call` 载荷的原值——绝不是展示标签，因此客户端不对其做本地化，未知名称按原样渲染。客户端通过投影 seam 的快照与变更流渲染全日志数字；参考消费者是 Web 聊天统计条，其窗口折叠以相同字段名充当无单元时的回退。
...
折叠状态保存八个总计——工具明细承载在 `tools` 这一总计里——外加进行中的边界：`lastTurn`（最近一次被计数 `step/end` 的轮次）、`openStep`（打开步的边界事实，由其 `assistant/message` 关闭）与 `pendingCalls`（按 callId 记录的分发时间与 `tool/call` 名称）。wire 视图是严格子集——八个总计——因此持久缓存的状态 schema 以边界字段扩展视图 schema。
...
- 工具时间按 callId 配对 `tool/call` → `tool/result`，并把同一增量记到该调用记录的 `name` 上；排序后的 `tools` 列表就地更新，因此折叠从不重放日志，且同一配对的两个数字始终一致。未解决的调用在 `turn/end` 时丢弃，因为结果总在其轮内落地，所以它们不会进入 `tools`；撞上 `Object` 原型名的 callId 读作未匹配。
...
- **工具名就是调用写入日志的名字**——`tools` 以 `tool/call` 的名称为键，因此改名或别名化的工具单独成条，始终未结算的调用是缺失而非零值，而单个工具内部的子调用（PTC）留在其父调用的数字里。
