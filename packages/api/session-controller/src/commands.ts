...
export class SessionCommandController {
  ...
  async delete(request: SessionDeleteRequest): Promise<SessionDeleteValue> {
    ...
    for (const id of cascade) {
      ...
      // Physically destroy the durable log. A pending creator handle owns the
      // artifact and would resurrect it on its next flush, so the provider
      // refuses; surface that as the same recoverable code creation uses
      ...
      try {
        ...
        if (error instanceof Error && error.name === 'SessionAlreadyOwnedError') {
          throw new RemoteError('session/writer-held', error.message, { sessionId: id })
        }
        throw error
      }
