...
- The resource snapshot contains only `status`, `value`, and `failure`; `value` is `WorkspaceFileStat` metadata. Content reads do not wait for the first metadata frame once the provider is available. A failed observation is announced over the loaded pages, with a reload the reader asks for; it never replaces loaded content by itself.
...
- **Reload** — the tab rereads through its Remote callbacks, preserving its scroll preference and retiring older requests. A write it did not make re-reads on its own once the observations stop arriving for 200 ms, merging the writes of one agent turn into a single read; a tab the reader cannot see waits until it is shown, and a renderer that wrote the file itself re-reads immediately through the `reload` owner prop. The change compares the read version and the observation captured at read start with later `resource.value.version`, so an already observed version is not a new change after refresh. Reads neither refresh shared metadata nor clear another tab's notice.
