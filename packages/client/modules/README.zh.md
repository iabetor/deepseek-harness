...
- **Bootstrap 与代码替换限制**——页面保留 modules bootstrap 和静态平台模块的身份。移除或替换 bootstrap 需要刷新页面；动态替换请求会报告页面本地错误，并保留其 fiber 与导出。携带此类变更的图会在任何 entry 被失效或拆除之前被拒绝，因此无法满足的图不会让页面丢失可用 UI。替换包代码及其所有现有消费者不属于普通启停同步。
