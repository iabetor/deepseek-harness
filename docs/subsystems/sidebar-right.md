...
Preview records its loaded version and the version observed when a read starts. A write the tab did not make re-reads that tab alone once observations settle, merging a burst into one read, and a tab the reader cannot see waits until it is shown; a renderer that wrote the file itself re-reads at once through its `reload` owner prop. Neither touches shared metadata or another tab's content. Reads are non-transactional; versions are opaque equality tokens, not ordered timestamps ([resource observation and Preview RPC](../../.agents/notes/implemented/architecture/2026-09-08-document-preview-operations.md)).
